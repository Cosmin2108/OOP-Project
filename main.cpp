...
int main()
{
	...
	if(strcmp(s,"Da")==0)
    {
	  ...
	  {  std::cin>>s;
         switch(atoi(s))
		 {
			 case 1:{ int dim=(*aux).getSize(); std::cout<<"Dimensiunea listei: "<<dim<<"\n"; break;}
   ...
   /*
    Object *p=new Punct(11.5,2.3);
    Object *p2=new Punct(1.5,2.3);
    ObArray *aux=static_cast<ObArray*>(arr);   ///*dynamic_cast<Obarray*>(arr) cast de la o clasa de baza la cea derivata; !PASTREZ
    (*aux).addOb(p);
    (*aux).addOb(p2);
    std::cout<<"Aici"<<*arr<<*p;
  */
